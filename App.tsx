...
const SHOPEE_LINKS = [
  "https://s.shopee.com.my/5L5itXU2Qb",
  "https://s.shopee.com.my/5q1zUSS8Pi"
];
...
const App: React.FC = () => {
  ...
  // Premium state starts as false on every refresh.
  ...
  // Logic to handle the temporary unlock after Shopee redirect
  useEffect(() => {
    const unlockPending = localStorage.getItem('premium_unlock_pending');
    
    if (unlockPending === 'true') {
      // Automatic detection: User just returned from the Shopee link
      setIsPremium(true);
      // CRITICAL: Remove the flag immediately. 
      // This ensures that if the user refreshes NOW, it will lock again.
      localStorage.removeItem('premium_unlock_pending');
    } else {
      setIsPremium(false);
    }
  }, []);
  ...
  const handleUnlockPremium = () => {
    ...
    // Set the flag that we are initiating the unlock process
    localStorage.setItem('premium_unlock_pending', 'true');
    // Redirect to Shopee
  ...
  }, [history, historyIndex, isPremium]);

  const undo = () => {
    if (isPremium && historyIndex > 0) {
  ...
  const redo = () => {
    if (isPremium && historyIndex < history.length - 1) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const urls = await generateMagicImage(state, isPremium);
  ...
  return (
    <div className="min-h-screen pb-20 selection:bg-[#bf953f] selection:text-black">
        ...
        >
          ...
          >
            ...
            <div className="relative flex-1 bg-black/40 border border-white/10 overflow-hidden shadow-2xl rounded-sm">
              <img src={previewImage} alt="Large Preview" className="w-full h-full object-contain" />
            </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-4">
            ...
            {!isPremium ? (
              <button 
                ...
                className="px-8 py-4 bg-white/5 border border-[#bf953f] text-[#bf953f] font-semibold tracking-widest uppercase text-xs hover:bg-[#bf953f]/10 transition-all duration-300 inline-flex items-center gap-2 group animate-pulse"
              >
                <Zap className="w-3 h-3 fill-current" />
                Unlock Premium
              </button>
            ) : (
              <div className="px-8 py-4 bg-[#bf953f]/10 border border-[#bf953f] text-[#fcf6ba] font-semibold tracking-widest uppercase text-xs inline-flex items-center gap-2">
      ...
      {/* Main Studio - Strictly Locked if not premium */}
      <main 
        id="studio" 
        ref={studioRef}
        className="relative max-w-6xl mx-auto px-6 space-y-32 pt-20"
      >
        {!isPremium && (
          <div className="absolute inset-0 z-50 flex flex-col items-center pt-60 text-center bg-black/40 backdrop-blur-md">
            <div className="sticky top-1/3 glass-card p-10 md:p-16 border-[#bf953f]/30 max-w-xl shadow-[0_0_50px_rgba(191,149,63,0.2)]">
                <Lock className="w-16 h-16 text-[#bf953f] mx-auto mb-8 animate-bounce" />
                <h2 className="serif text-4xl gold-text mb-6 font-bold">Studio Locked</h2>
                <p className="text-zinc-300 text-lg mb-10 leading-relaxed font-light">
                  Akses penuh ke Studio Magic Affiliate Pro memerlukan keahlian Premium. 
                  Sila tekan butang di bawah untuk melawat pautan affiliate dan buka kandungan Premium secara automatik.
                </p>
                <button 
                  onClick={handleUnlockPremium}
                  className="w-full py-5 bg-[#bf953f] text-black font-bold uppercase tracking-[0.3em] text-sm hover:bg-[#fcf6ba] transition-all shadow-[0_0_30px_rgba(191,149,63,0.4)] flex items-center justify-center gap-3 active:scale-95"
                >
                  <Zap className="w-5 h-5 fill-current" />
                  Unlock Premium Sekarang
                </button>
                <p className="mt-6 text-[10px] text-zinc-500 uppercase tracking-widest font-medium">Sila ambil perhatian: App akan dikunci semula setiap kali halaman disegarkan semula.</p>
            </div>
          </div>
        )}

        <div className={!isPremium ? "blur-2xl pointer-events-none select-none opacity-10" : "transition-all duration-1000"}>
            {/* Step 1: Upload */}
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-6">
                          {state.customModelImage ? (
                            ...
                            <div 
                              onClick={() => modelInputRef.current?.click()}
                              className="w-32 h-40 border-2 border-dashed border-white/10 bg-white/5 flex flex-col items-center justify-center cursor-pointer hover:border-[#bf953f] transition-all group"
                            >
            ...
            <section className="flex flex-col items-center py-20 border-t border-white/10">
              ...
              <p className="mt-8 text-[#bf953f] text-sm font-bold uppercase tracking-widest">
                Premium Studio Render Active
              </p>
            </section>
